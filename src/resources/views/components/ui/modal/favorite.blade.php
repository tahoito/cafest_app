...
<div x-data x-cloak {{ $attributes }}>
  <template x-teleport="body">
        ...
        >
          <div class="flex items-center justify-end pt-3">
            <button
              ...
              @click.stop="$store.favModal.openCreate({{ $storeId }})"
            >
              新しいコレクション
            </button>
          </div>
    ...
    <div
      ...
      @click.self="$store.favModal.closeCreate({{ $storeId }})"
    >
      <div
        x-show="$store.favModal.createStoreId === {{ $storeId }}"
        x-transition
        class="fixed left-1/2 top-1/2 z-[221] w-[353px] h-[550px]
          -translate-x-1/2 -translate-y-1/2 bg-base px-5 py-5
      ...
      >
        <div class="flex items-start justify-between">
          <button
            ...
            @click="$store.favModal.closeCreate({{ $storeId }})"
            aria-label="閉じる"
          >
            <x-icons.close class="w-6 h-6 text-text_color" />
          </button>
